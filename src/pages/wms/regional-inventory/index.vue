...
<template>
  <VCard>
    ...
    <DynamicFilter
      title="Data Inventori Regional"
      :fields="filterConfig.fields"
      :field-configs="filterConfig.fieldConfigs"
      quick-search-placeholder="Cari SKU, nama produk, atau regional warehouse..."
      :quick-search-fields="['product.sku', 'product.name', 'regional_warehouse.name', 'regional_warehouse.code', 'regional_warehouse.region']"
    ...
    >
      ...
      <template #item.product="{ item }">
        <VBtn
          ...
          :to="{ name: 'wms-regional-inventory-id', params: { id: item.id } }"
        >
      ...
      <template #item.regional_warehouse="{ item }">
        <div v-if="item?.regional_warehouse">
          <div class="font-weight-medium">{{ item.regional_warehouse.name }}</div>
          <div class="text-body-2 text-medium-emphasis">{{ item.regional_warehouse.code }} - {{ item.regional_warehouse.region }}</div>
        </div>
        <span v-else class="text-medium-emphasis">-</span>
      </template>
...
const headers = [
  ...
  { title: 'Regional Warehouse', key: 'regional_warehouse' },
...
async function fetchInventoryItems() {
  ...
  try {
    ...
    const res = await $api('/wms/regional-inventory/paginated', {
      method: 'POST',
      body: requestBody,
    })
    ...
    // Fetch regional warehouse details for each inventory item
...
async function fetchRegionalWarehouseDetails() {
  try {
    ...
    console.log('🔍 Sample item regional_warehouse_id:', sampleItem?.regional_warehouse_id)
    console.log('🔍 Sample item regional_warehouse:', sampleItem?.regional_warehouse)
    console.log('🔍 All items regional_warehouse_id values:', inventoryItems.value.map(item => item.regional_warehouse_id))
    
    // Check if warehouse data is already included
    const itemsWithWarehouseData = inventoryItems.value.filter(item => item.regional_warehouse)
    console.log('🔍 Items with existing warehouse data:', itemsWithWarehouseData.length)
    
    // Get unique regional warehouse IDs
    const warehouseIds = [...new Set(inventoryItems.value
      .filter(item => item.regional_warehouse_id)
      .map(item => item.regional_warehouse_id))]
    
    console.log('📋 Unique warehouse IDs to fetch:', warehouseIds)
    console.log('🔍 Items with warehouse IDs:', inventoryItems.value.filter(item => item.regional_warehouse_id))
    ...
    for (const warehouseId of warehouseIds) {
      try {
        const res = await $api(`/wms/regional-warehouses/${warehouseId}`)
    ...
    inventoryItems.value = inventoryItems.value.map(item => ({
      ...item,
      regional_warehouse: warehouseDetails[item.regional_warehouse_id] || item.regional_warehouse || null
    }))
