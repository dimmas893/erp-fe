      ...
      <div class="d-flex align-center gap-3">
        ...
        <div>
          <h2 class="text-h5 mb-1">
            Tambah Inventori Pusat
          </h2>
          <p class="text-body-2 text-medium-emphasis mb-0">
            Tambahkan inventori baru untuk central warehouse
          </p>
        </div>
      </div>
      <VBtn
...
async function handleSubmit() {
  ...
  try {
    ...
    console.log('📤 Creating central inventory with data:', requestBody)

    const res = await $api('/wms/central-inventory', {
      method: 'POST',
      body: requestBody,
    })
    ...
    await showSuccessAlert({
      title: 'Berhasil',
      text: 'Inventori pusat berhasil ditambahkan',
    })

    // Redirect to index page
    router.push({ name: 'wms-central-inventory' })
  } catch (error) {
    console.error('❌ Error creating central inventory:', error)
    ...
    if (error.response?.data?.errors) {
      ...
      await showErrorAlert(error, {
        ...
        text: 'Tidak dapat menambahkan inventori pusat. Silakan coba lagi.',
      })
    }
  } finally {
    loading.value = false
